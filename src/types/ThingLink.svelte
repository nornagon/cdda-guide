...
if (item?.type === 'vehicle_part' && !item.name && item.item) item = data.byId('item', item.item)
</script>

{#if count != null}
<span style="white-space: nowrap">
{#if !countsByCharges(item)}{countToString(count)}{/if}
<svelte:self type={type} id={id} plural={countIsPlural(count) && !countsByCharges(item)} />{#if countsByCharges(item)}{' '}({countToString(count)}){/if}</span>
{:else}
