...
<section>
  <dl>
    ...
    <dt>Excess</dt>
    <dd>
      <ul class="comma-separated">{#each excessNames as n}<li>{singular(n)}</li>{/each}</ul>
    </dd>
    {/if}
    {#if item.deficiency}
    <dt>Deficiency</dt><dd><ul class="comma-separated">{#each deficiencyNames as n}<li>{singular(n)}</li>{/each}</ul></dd>
    {/if}
