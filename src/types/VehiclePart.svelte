...
import { asKilograms, asLiters, CddaData, singular, singularName } from "../data";
...
function bonusLabel(item: VehiclePart) {
  const light_flags = [
    'LIGHT', 'CONE_LIGHT', 'WIDE_CONE_LIGHT', 'CIRCLE_LIGHT', 'DOME_LIGHT',
    'AISLE_LIGHT', 'EVENTURN', 'ODDTURN', 'ATOMIC_LIGHT',
  ]
  const flags = item.flags ?? []
  if (flags.includes('SEATBELT'))
    return 'Strength'
  else if (flags.includes('HORN'))
    return 'Noise'
  else if (flags.includes('MUFFLER'))
    return 'Noise Reduction'
  else if (flags.includes('VISION'))
    return 'Range'
  else if (light_flags.some(lf => flags.includes(lf)))
    return 'Light'
  else if (flags.includes('REAPER'))
...
<h1>{item.name ? singularName(item) : singularName(data.byId('item', item.item))}</h1>
...
<section>
  ...
  <dl>
    ...
    {/if}
    <dt title="Maximum damage part can sustain before being destroyed">Durability</dt>
    ...
    {#each item.qualities as [quality, level]}
      <li>Has level <strong>{level ?? 1} <ThingLink type="tool_quality" id={quality} /></strong> quality.</li>
    {/each}
    </ul>
    </dd>
    {/if}
  ...
  <p style="color: var(--cata-color-gray)">{singular(item.description ?? data.byId('item', item.item).description)}</p>
</section>
...
<section>
  ...
  <dl>
    ...
    <RequirementData requirement={{...item.requirements?.install ?? {}, components: (item.requirements?.install?.components ?? []).concat([[[item.item, 1]]])}} />
  </dl>
</section>
...
<section>
  ...
  <dl>
    ...
    <dt title="Time required goes down with better skills">Time</dt>
    <dd>{item.requirements?.removal?.time ?? asMinutes(`${parseDuration(item.requirements?.install?.time ?? '1 hour') / 2} s`)}</dd>
